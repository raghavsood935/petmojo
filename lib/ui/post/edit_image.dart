...
//Bottom Buttons
  ...
  Future<bool> _onWillPop() async {
    return (await showDialog(
      ...
      builder: (context) => new AlertDialog(
        ...
        content: new Text('If you go back now you will lose all of the edits you have made.'),
        actions: <Widget>[
          TextButton(
                        ...
                        children: [
                          Visibility(
                            ...
                            child: Center(
                              child: Padding(
                                ...
                                child: Icon(Icons.cancel_outlined,size: 70,color: Colors.red,),
                              ),
                            ),
                          ),
                          Spacer(),
                          Row(
                              children: [
                                ...
                                IconButton(
                                  onPressed:(){ setState(() {
                                    items.add(WidgetModel(offset: Offset(width/2-90,height/2), text: '4', type: false));
                                  });},
  ...
  void addEmoji(int x) {
    setState(() {
      items.add(WidgetModel(offset: Offset(100,100), text: '$x', type: true));
    });
  }
  ...
  void _presentGridPopup(BuildContext context) {
    Navigator.of(context).push<void>(PageRouteBuilder(
      ...
      pageBuilder: (context, animation, secondaryAnimation) => GridPopup(callback: (val)=> addEmoji(val),),
  ...
  Function onDragStart(int index) => (x, y) {
    setState(() {
      if(!items[index].type)
        items[index] = items[index].copyWithNewOffset(Offset(x-90, y-15));
      else
  ...
  Function onDragEnd(int index) => () {
    ...
    setState(() {
      if(x1>((width-70)/2)  && x1<((width-70)/2 +70)  && y1<120 )
