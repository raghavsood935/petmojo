...
class DTUpgradePaymentViewModel extends FutureViewModel<void>
  ...
  DTUpgradePaymentViewModel(this.amount, this.bookingId);
    ...
    try {
      if (await Util.checkInternetConnectivity()) {
        ...
        BaseResponse<SendDataResponse> result = await runBusyFuture(
            _tamelyApi.setUpgradePaymentDetailsTraining(setPaymentDetailsBody),
            throwException: true);
        notifyListeners();
      } else {
        snackBarService.showSnackbar(message: "No Internet connection");
